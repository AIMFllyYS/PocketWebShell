...
- **桌面模式对齐 Chrome RDS（0.1.58–0.1.59 加固）**：`setDesktopMode` 写入桌面 UA + 版本对齐的 UA-CH metadata（Chromium/151 brand 列表），并按 `viewWidth * 100 / 980` 调用 `WebView.setInitialScale`（物理像素口径，范围 25–250，不钳 100 以铺满宽屏）；浏览标签不再额外 `reload()`，由一次 `reconfigure` / `pendingDesktopReload` 刷新——且桌面变更走绕 HTTP 缓存的 reload（`reloadBypassingCache`，主文档强制重新请求，`onPageStarted` 恢复 `LOAD_DEFAULT`）。document-start bootstrap：改写**所有** viewport meta（Blink 后解析者覆盖先前者，只改第一个等于没改）、无 meta 时补建且只在 `head`/`documentElement` 存在时挂接（禁止向 document 根追加）、`__wsBoot` 幂等守卫、MutationObserver 挂 document 根 + DOMContentLoaded 重跑、`window.__wsForceZoom` 暴露给宿主在 `onPageFinished` 强制重断；桌面模式页面在 `onPageStarted` 全量补注一次，不支持 `DOCUMENT_START_SCRIPT` 的老 WebView 再在 `onPageFinished` 全量兜底。页面完成后写入桌面探针日志（innerWidth/clientWidth/DPR/vp980/boot）。本地导入不改 layout 宽，且浏览菜单/悬浮球的「桌面版」入口对本地导入禁用。
