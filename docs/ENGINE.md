...
- **桌面模式对齐 Chrome RDS（0.1.58 加固）**：`setDesktopMode` 写入桌面 UA + 版本对齐的 UA-CH metadata（Chromium/151 brand 列表），并按 `viewWidth * 100 / 980` 调用 `WebView.setInitialScale`；浏览标签不再额外 `reload()`，由一次 `reconfigure` / `pendingDesktopReload` 刷新——且桌面变更走绕 HTTP 缓存的 reload（`reloadBypassingCache`，主文档强制重新请求，`onPageStarted` 恢复 `LOAD_DEFAULT`）。document-start bootstrap 带 `(head||documentElement||document)` 兜底、try 包裹、`document` 根观察器与 DOMContentLoaded 重跑；不支持 `DOCUMENT_START_SCRIPT` 的老 WebView 在 `onPageStarted` 降级注入。本地导入不改 layout 宽，且浏览菜单/悬浮球的「桌面版」入口对本地导入禁用。
