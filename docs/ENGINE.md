...
- **桌面模式对齐 Chrome RDS（0.1.58–0.1.59 加固，0.1.61 修复身份链）**：`setDesktopMode` 写入桌面 UA + 版本对齐的 UA-CH metadata（构造抽为 `WebEngineDefaults.userAgentMetadata` 纯函数并有单元测试——`BrandVersion` 的 brand/majorVersion/fullVersion 三字段缺一必抛 `IllegalStateException`，0.1.58–0.1.60 因 GREASE 品牌漏填 fullVersion 导致 `setUserAgentMetadata` 从未执行；桌面身份对齐 Windows/x86/64 位，支持 `USER_AGENT_METADATA_FORM_FACTORS` 时同步声明 Desktop/Mobile；设置后回读 `getUserAgentMetadata` 校验写日志，失败记 ERROR 不再静默），并按 `viewWidth * 100 / 980` 调用 `WebView.setInitialScale`（物理像素口径，范围 25–250，不钳 100 以铺满宽屏）；浏览标签不再额外 `reload()`，由一次 `reconfigure` / `pendingDesktopReload` 刷新——且桌面变更走绕 HTTP 缓存的 reload（`reloadBypassingCache`，主文档强制重新请求，`onPageStarted` 恢复 `LOAD_DEFAULT`）。document-start bootstrap：改写**所有** viewport meta（Blink 后解析者覆盖先前者，只改第一个等于没改）、无 meta 时补建且只在 `head`/`documentElement` 存在时挂接（禁止向 document 根追加）、`__wsBoot` 幂等守卫、MutationObserver 挂 document 根 + DOMContentLoaded 重跑、`window.__wsForceZoom` 暴露给宿主在 `onPageFinished` 强制重断；桌面模式页面在 `onPageStarted` 全量补注一次，不支持 `DOCUMENT_START_SCRIPT` 的老 WebView 再在 `onPageFinished` 全量兜底。页面完成后写入桌面探针日志（身份链 uad + 布局链 iw/cw/vps/mq 断点/visualViewport/boot，验收两条链路分开看）。本地导入不改 layout 宽，且浏览菜单/悬浮球的「桌面版」入口对本地导入禁用。
