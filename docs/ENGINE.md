...
- **下载不再只认导航型 `DownloadListener`**：`DownloadPolicy.looksLikeFileDownload` 把带常见文件后缀的主框架 URL 交给 `DownloadSink` / 系统 `DownloadManager`，而不是当网页打开。`loadUrl()` 不走 `shouldOverrideUrlLoading`，因此 `loadUrlOrPrompt` 必须同样拦截（检查更新打开 GitHub APK 即此路径）。Blob 先取元数据再按块经 `WebMessageListener` 写出，避开 `evaluateJavascript` 的 Binder 上限；`onPageStarted` 不再在 blob / data URL 上取消正在写入的文件。
...
- **桌面模式对齐 Chrome RDS（0.1.58–0.1.59 加固，0.1.61 修复身份链，0.1.62 布局宽 980→1280）**：`setDesktopMode` 写入桌面 UA + 版本对齐的 UA-CH metadata（构造抽为 `WebEngineDefaults.userAgentMetadata` 纯函数并有单元测试——`BrandVersion` 的 brand/majorVersion/fullVersion 三字段缺一必抛 `IllegalStateException`，0.1.58–0.1.60 因 GREASE 品牌漏填 fullVersion 导致 `setUserAgentMetadata` 从未执行；桌面身份对齐 Windows/x86/64 位，支持 `USER_AGENT_METADATA_FORM_FACTORS` 时同步声明 Desktop/Mobile；设置后回读 `getUserAgentMetadata` 校验写日志，失败记 ERROR 不再静默；**不支持 UA-CH 的老/定制 WebView 如 HwWebview 114 记一条 INFO，身份链仅剩 UA 字符串**），并按 `viewWidth * 100 / DESKTOP_VIEWPORT_WIDTH` 调用 `WebView.setInitialScale`（物理像素口径，范围 25–250，不钳 100）。布局宽取 **1280** 而非 Chrome RDS 的 980：注入式方案无法像内核那样禁用站点 viewport meta，而现代响应式断点普遍 ≥992（Bootstrap lg）/1024（Tailwind lg）/1012（GitHub），真机实测（HwWebview 114）布局宽 980 时连 `(min-width:980px)` 都不命中——980 只能得到窄桌面/平板布局。浏览标签不再额外 `reload()`，由一次 `reconfigure` / `pendingDesktopReload` 刷新——且桌面变更走绕 HTTP 缓存的 reload（`reloadBypassingCache`，主文档强制重新请求，`onPageStarted` 恢复 `LOAD_DEFAULT`）。document-start bootstrap：改写**所有** viewport meta（Blink 后解析者覆盖先前者，只改第一个等于没改）、无 meta 时补建且只在 `head`/`documentElement` 存在时挂接（禁止向 document 根追加）、`__wsBoot` 幂等守卫、MutationObserver 挂 document 根 + DOMContentLoaded 重跑、`window.__wsForceZoom` 暴露给宿主在 `onPageFinished` 强制重断；桌面模式页面在 `onPageStarted` 全量补注一次，不支持 `DOCUMENT_START_SCRIPT` 的老 WebView 再在 `onPageFinished` 全量兜底。页面完成后写入桌面探针日志（身份链 uad + 布局链 iw/cw/ow/rc/vps/mq 断点/visualViewport/boot，两条链路分开验收）。本地导入不改 layout 宽，且浏览菜单/悬浮球的「桌面版」入口对本地导入禁用。
- **证书对话框不离开页面**：`onPageStarted` 仍会 cancel 未决 handler；用户点取消只关掉对话框，不再 `closeTab` / 退出站点壳。从不默认 `SslErrorHandler.proceed()`。
- **顶层 HTTP（0.1.63）**：网络安全配置允许明文，但主框架 `http://` 必须经可见宿主确认（`onCleartextPrompt` → `AppConfirmDialog`）。继续后按会话+host 记住，混合内容仍 `MIXED_CONTENT_NEVER_ALLOW`，证书路径不变。`loadUrl` 不走 `shouldOverrideUrlLoading`，闸门同时拦引擎加载入口与主框架导航回调。
...
## 12. 0.1.47–0.1.53 开屏重构与 Markdown 站点壳

- **开屏（0.1.53）**：废弃黑洞引力与主线程粒子模拟，重构为原生级极简品牌星核微动效（`BrandMark` 太极环与鎏金星核）。采用双层并行流水线：底层 `MainScaffold` 在开屏期间并行完成布局与数据预取（期间 `suppressLiveGlass` 禁用实时毛玻璃），开屏纯通过 GPU 硬件加速 `graphicsLayer`（Scale + Alpha）绽放淡出（总耗时约 550ms），彻底消灭主线程首帧排版撞车与丢帧卡顿。同时配置 Android 12+ (API 31+) 原生 `windowSplashScreen` 契约，消除窗口冷启动视觉闪烁。
