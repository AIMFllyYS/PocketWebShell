...
- 引入前确认：开源协议与本项目 `GPL-3.0-or-later` 兼容（Apache-2.0 / MIT / BSD / LGPL 等可进入组合成品；禁止引入 GPL 不兼容的依赖）、维护活跃、支持 minSdk 29。
...
- 主屏 Dock 为 88dp 高四图标托盘；功能页切为 66dp 高标签栏；主屏编辑态该托盘换成 §6.3 统一菜单栏，不叠第二条底栏。系统导航安全区单独计算，禁止用硬编码 96dp 替代真实 inset。壁纸位于内容 padding 之外并覆盖整个 Dock 采样源。
...
**双指捏合（内划）**进入编辑模式；双指平行滑动也可进入（只进不退）。**双指外扩 / 完成 / 返回手势**退出。

进入后主屏进入系统性整理界面，所有可点操作只出现在 **一条 iOS 风格统一菜单栏** 里，位置与主屏 Launcher Dock 相同（底栏安全区 + 水平外边距），并 **替换** 四图标 Dock，不在顶部另做完成胶囊，不在网格内再铺第二行动作：

- 进入时所有图标做 iOS 式小幅旋转抖动（±1.6°，130ms 往返）；非编辑态不挂无限抖动时钟；
- 每个图标左上角圆形勾选角标（画在图标槽内，不画出邻格）：选中 `primary` 实心 + 对勾，未选中半透 `surface` + 描边空心；
- 统一菜单栏（唯一一处 live blur，`glassSurface` / Haze，半径预算同 Dock）：
  - 完成（左，可见文字「完成」）；
  - 选中计数（中，「已选 n / m」）；
  - 桌面滑动切换（右，`homeScrollMode`：左右翻页 / 上下滚动，写入既有 DataStore，不改 page/slot 模型）；
  - 批量：删除、移出文件夹、移入文件夹、全选/取消全选；
- 栏内文案一律单行省略；破坏性「删除」用 `error`；触控目标至少 48dp；
- 编辑模式下点按切换勾选；已选中的多个图标按住其中一个、位移超过 touchSlop 即汇聚成组拖动（不弹长按菜单）；
- 单元格标题单行省略、宽度受槽约束，禁止画进邻格；大字号不够高时整页滚动（`requiresVerticalScroll`），禁止靠标签折行抢邻槽。
