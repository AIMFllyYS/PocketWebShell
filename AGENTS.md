...
- `docs/WEBSITE.md`: product-website sync and deployment — after every published GitHub Release, the separate website project (`../PocketWebShell-site/`, `https://xuanlan.1037solo.com`) must be updated per its checklist (changelog snapshot, generated release data, `lib/product.ts`, scattered version strings, verification assertions), pass all checks, then be zipped from `dist/client/` and deployed at the domain root. Debug iterations never trigger a website sync.
